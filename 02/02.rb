...
array.each { |s|
  code = s.split(' ')
  min = code[0].split('-')[0].to_i
  max = code[0].split('-')[1].to_i
  letter = code[1].chars[0]
  password = code[2].strip.split('')
  letter_count = password.count { |l| l == letter}

  count += 1 if letter_count >= min && letter_count <= max
}
# partie 2
count = 0
array.each { |s|
  ...
  if password[min-1] == letter &&  password[max-1] != letter
    count += 1
  elsif password[min-1] != letter &&  password[max-1] == letter
    count += 1
  end
}
p count
