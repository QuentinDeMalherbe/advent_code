...
array_main_cond = []
array.each do |s|
  count += 1 if s.split(" ").count == 8
  array_main_cond << s if s.split(" ").count == 8
  count += 1 if s.split(" ").count == 7 && s['cid'].nil?
  array_main_cond << s if s.split(" ").count == 7 && s['cid'].nil?
end
# parite 2
count = 0
array_main_cond.each do |s|
  checked_array = []
  s.split(' ').each do |w|
    if w.split(':').first == 'byr'
      checked_array << true if w.split(':').last.scan(/^\d{4}$/).count !=0 && w.split(':').last.to_i > 1920 && w.split(':').last.to_i < 2002
    elsif w.split(':').first == 'iyr'
      checked_array << true if w.split(':').last.scan(/^\d{4}$/).count !=0 && w.split(':').last.to_i > 2010 && w.split(':').last.to_i < 2020
    elsif w.split(':').first == 'eyr'
      checked_array << true if w.split(':').last.scan(/^\d{4}$/).count !=0 && w.split(':').last.to_i > 2020 && w.split(':').last.to_i < 2030
    elsif w.split(':').first == 'hgt'
      checked_array << true if w.split(':').last.scan(/^\d{3}cm$/).count !=0 && w.split(':').last[0..2].to_i > 150 && w.split(':').last[0..2].to_i < 193
      checked_array << true if w.split(':').last.scan(/^\d{2}in$/).count !=0 && w.split(':').last[0..1].to_i > 59 && w.split(':').last[0..1].to_i <76
    elsif w.split(':').first == 'hcl'
      checked_array << true if w.split(':').last.scan(/^#.{6}$/).count !=0
    elsif w.split(':').first == 'ecl'
      checked_array << true if ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].include?(w.split(':').last)
    elsif w.split(':').first == 'pid'
      checked_array << true if w.split(':').last.scan(/^\d{9}$/).count != 0
    end
    count += 1 if checked_array.count == 7
  end
end
p array_main_cond.count
p count
